    ...
    function showData() {
      ...
      firebase.database().ref().child('events/soc').limitToLast(3).once('value', function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
          ...
          img1 = 'carImage1'+i;
          img2 = 'carImage2'+i;
          firebase.storage().ref('soc').child(childSnapshot.val().name + '/image0').getDownloadURL().then(function (url) {
            document.getElementById(img0).src = url;
          });
          firebase.storage().ref('soc').child(childSnapshot.val().name + '/image1').getDownloadURL().then(function (url) {           
            document.getElementById(img1).src = url;
          });
          firebase.storage().ref('soc').child(childSnapshot.val().name + '/image2').getDownloadURL().then(function (url) {
    ...
    function data(dataKey) {
      ...
      firebase.database().ref('/events/soc/' + dataKey).once('value').then(function (snapshot) {
        ...
        firebase.storage().ref('soc').child(snapshot.val().name + '/image0').getDownloadURL().then(function (url) {
          document.getElementById('carImage0').src = url;
        });
        firebase.storage().ref('soc').child(snapshot.val().name + '/image1').getDownloadURL().then(function (url) {
          document.getElementById('carImage1').src = url;
        });
        firebase.storage().ref('soc').child(snapshot.val().name + '/image2').getDownloadURL().then(function (url) {
