function showData() {
    var i = 1;
    firebase.database().ref().child('events/sports').limitToLast(3).once('value', function (snapshot) {
  ...
  function showNavBar() {
    var i = 1;
    firebase.database().ref().child('events/sports').once('value', function (snapshot) {
      snapshot.forEach(function (childSnapshot) {
  ...
  function data(dataKey) {
    console.log(dataKey);
    firebase.database().ref('/events/sports/' + dataKey).once('value').then(function (snapshot) {
